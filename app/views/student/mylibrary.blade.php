...
@section('mylibrary.nav') selected="selected" @stop
@section('mylibrary.menu') active @stop
...
function saveRef(button){
    ...
    if(error==""){
		...
		$.ajax({  
			type: 'POST',  
			url: '<?php echo URL::to('/'); ?>/student/reference/create',
...
function load(){
    if(loading==false){
        ...
        $.ajax({  
            type: 'GET',  
            url: '<?php echo URL::to('/'); ?>/student/reference/get'+search,
            contentType: 'application/json',
            success: function(data){
                loading=false;
...
function delRef(){
    ...
    if(deleted.length>0){
        ...
        $.ajax({  
            type: 'POST',  
            url: '<?php echo URL::to('/'); ?>/student/reference/del',  
            data: JSON.stringify(model),  
            dataType: 'text',  
            contentType: 'application/json',
            success: function(data){
